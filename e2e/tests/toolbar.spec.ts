...
test.describe(`LaunchDarkly Toolbar - ${config.testEnv} environment`, () => {
  ...
  test('should support drag and snap functionality', async ({ page }: { page: Page }) => {
    const iframeSelector = 'iframe[title="storybook-preview-iframe"]';
    const iframe = page.frameLocator(iframeSelector);
    const toolbarContainer = iframe.getByTestId('launchdarkly-toolbar').first();

    // Setup: Get dimensions and coordinates
    const [iframeBox, toolbarBox, iframeElement] = await Promise.all([
      page.locator(iframeSelector).boundingBox(),
      toolbarContainer.boundingBox(),
      page.frame('storybook-preview-iframe'),
    ]);

    if (!iframeBox) throw new Error('Could not get iframe bounding box');
    if (!toolbarBox) throw new Error('Could not get toolbar bounding box');
    if (!iframeElement) throw new Error('Could not find iframe');

    const screenWidth = await iframeElement.evaluate(() => window.innerWidth);
    const startY = toolbarBox.y + toolbarBox.height / 2;
    const centerX = screenWidth / 2;

    // Focus iframe and verify initial position
    await toolbarContainer.click();
    // Give time for CSS classes to be applied
    await expect(toolbarContainer).toHaveClass(/positionRight/, { timeout: 100 });

    // Enable drag mode with Control key
    await page.keyboard.down('ControlOrMeta');
    await toolbarContainer.hover();

    // Verify toolbar doesn't expand in drag mode
    await expect(iframe.getByRole('tab', { name: 'Flags' })).not.toBeVisible();

    // Drag to 1 pixel left of center (should snap to left)
    await page.mouse.move(toolbarBox.x + toolbarBox.width / 2, startY);
    await page.mouse.down();
    await page.mouse.move(iframeBox.x + centerX - 1, startY);
    await page.mouse.up();
    await expect(toolbarContainer).toHaveClass(/positionLeft/);

    // Drag to center (should snap back to right)
    const newToolbarBox = await toolbarContainer.boundingBox();
    if (!newToolbarBox) throw new Error('Could not get updated toolbar bounding box');

    await page.mouse.move(newToolbarBox.x + newToolbarBox.width / 2, startY);
    await page.mouse.down();
    await page.mouse.move(iframeBox.x + centerX, startY);
    await page.mouse.up();
    await expect(toolbarContainer).toHaveClass(/positionRight/);
